export type NodeAssetTarget = {
  // ── ANDROIDTUI: "android" covers Termux on-device builds ────────────────
...
const NATIVE_FILE_NAMES = {
  ...
  linux: "libopentui.so",
  win32: "opentui.dll",
  // ── ANDROIDTUI: Android/Termux — Bionic ELF, same extension as linux ────
  android: "libopentui.so",
} as const

// ── ANDROIDTUI: Android natives ship under the @androidtui scope ──────────────
...
// publishes it as @androidtui/core-android-arm64.
const ANDROIDTUI_SCOPE = "@androidtui/core-android"
...
export function getNativeAssetDescriptor(target: NodeAssetTarget): NativeAssetDescriptor {
  ...
  // ── ANDROIDTUI: Android natives come from @androidtui, not @opentui ──────────
  const packageName =
    target.platform === "android"
      ? `${ANDROIDTUI_SCOPE}-${target.arch}`
...
export function getCurrentNodeAssetTarget(): NodeAssetTarget {
  ...
  // ── ANDROIDTUI: detect Termux before the libc check ──────────────────────
