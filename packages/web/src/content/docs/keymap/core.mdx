...
| `resolveBindingSections(config)` | Converts sectioned command config into binding arrays plus `get(section, cmd)`, `pick(...)` and `omit(...)` helpers |
...
`resolveBindingSections()` is an opinionated config-to-keymap transformation for app-level config split into named sections. It is one practical shape; copy and adjust it if your app needs different rules. Values can be `false`, `"none"`, a key, a full binding object, or arrays of keys/binding objects. `false`, `"none"`, and empty arrays emit no bindings. The command name comes from the record key.
...
resolved.pick("prompt_input", ["prompt.history.previous", "prompt.history.next"])
...
| API                       | Description                                                                                                       |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `sections[section]`       | All enabled bindings for the section, preserving section config order                                             |
...
`pick()` and `omit()` use exact command names and do not trim. They are meant for hard-coded application command lists. Use them when one semantic config section feeds multiple activation layers.
