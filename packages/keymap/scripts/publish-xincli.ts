// publish-xincli.ts — Publish @xincli/opentui-keymap
//
// This script builds the keymap package (same as `bun scripts/build.ts`),
// then repackages the dist/ output as @xincli/opentui-keymap with the
...
// package.json — @xincli/opentui-keymap directly depends on
...
console.log("==========================================")
console.log(`Publishing @xincli/opentui-keymap@${VERSION}`)
console.log("==========================================")
...
// --- Step 3: Repackage as @xincli/opentui-keymap ---------------------------
console.log("\n=== Step 2: Repackage as @xincli/opentui-keymap ===")

const distPkg: PackageJson = JSON.parse(readFileSync(distPkgPath, "utf8"))

// Rewrite the package identity
distPkg.name = "@xincli/opentui-keymap"
distPkg.version = VERSION
distPkg.description = "XINCLI fork of OpenTUI keymap — for use with @xincli/opentui-core"
distPkg.repository = {
  type: "git",
...
// Also alias @opentui/react and @opentui/solid if present (keymap has these as peers)
if (distPkg.peerDependencies && distPkg.peerDependencies["@opentui/react"]) {
  distPkg.peerDependencies["@opentui/react"] = `npm:@xincli/opentui-react@${VERSION}`
}
if (distPkg.peerDependencies && distPkg.peerDependencies["@opentui/solid"]) {
  distPkg.peerDependencies["@opentui/solid"] = `npm:@xincli/opentui-solid@${VERSION}`
}
if (distPkg.devDependencies && distPkg.devDependencies["@opentui/react"]) {
  distPkg.devDependencies["@opentui/react"] = `npm:@xincli/opentui-react@${VERSION}`
}
if (distPkg.devDependencies && distPkg.devDependencies["@opentui/solid"]) {
  distPkg.devDependencies["@opentui/solid"] = `npm:@xincli/opentui-solid@${VERSION}`
}
...
console.log(`       peerDependencies: ${JSON.stringify(distPkg.peerDependencies, null, 2)}`)
...
if (!SHOULD_PUBLISH) {
  ...
  console.log(`\nArtifact:`)
  console.log(`  ${artifactsDir}/xincli-opentui-keymap-${VERSION}.tgz`)
  process.exit(0)
}
...
const tgzPath = join(artifactsDir, `xincli-opentui-keymap-${VERSION}.tgz`)
...
if (publishResult.error || publishResult.status !== 0) {
  console.error(`FAIL: npm publish failed for @xincli/opentui-keymap@${VERSION}`)
  process.exit(1)
}

console.log(`\n==========================================`)
console.log(`SUCCESS: @xincli/opentui-keymap@${VERSION} published!`)
console.log(`==========================================`)
console.log(`\nInstall with:`)
console.log(`  npm install @xincli/opentui-keymap@${VERSION} --legacy-peer-deps`)
