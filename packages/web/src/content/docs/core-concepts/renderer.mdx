...
// Target specific OSC 52 selections
renderer.copyToClipboardOSC52("primary", ClipboardTarget.Primary)
renderer.copyToClipboardOSC52("secondary", ClipboardTarget.Secondary)
```

`ClipboardTarget` provides `Clipboard`, `Primary`, `Select`, and `Secondary`,
which map to the OSC 52 `c`, `p`, `s`, and `q` selection bytes respectively.
Both methods return `false` when capability policy blocks the operation or
OpenTUI cannot construct the sequence, for example after renderer destruction.
A `true` result means the renderer output path was invoked; it does not prove
